...
class Parsers extends parserTools {
    ...
    parseYifyWithoutApi (start, film, callback) {
		...
		if (!res && typeof res.body === 'undefined') return callback([]);
        ...
        promise
            ...
            let qFilmItem = async.queue(function (url, callback) {
                needle.get(url, (err, res) => {
                    if (url === 'http://yify.is/movie/view' ||!res || typeof res.body === 'undefined') return callback([]) ;
    ...
    parseYVifyAPI (start, film, callback) {
        ...
        function recursiveApiSearch (api, film, start) {
            ...
            needle.get(url, (err, res) => {
                if (res && typeof res.body !== 'undefined' && res.body.status == 'ok' && res.body.data.movie_count > 0)
            for (var i = 0; i < res.body.data.movies.length; i++) {
                let data = {
                    ...
                    magnt_1080	: res.body.data.movies[i].torrents[2],
